...
\begin{document}
	...
	\rem{14 Apr 2021 Bumping to Beta since we played this in the JLall3. Deleting old notes from main branch.}


	
	\begin{bidtable}{\orauction{1c}}
		\di1 & No change, 0-7(8) \\
		\he1 & No changes, UPH = extras w/o primary hearts, PH = clubs \\
		\sp1 & Balanced or \hhh. Over 1NT waiting \cl2=\hhh, \di2/\hhh/\sss are the 4\hhh/4\sss/no 4CM bal hands. \\
		1NT & \sss \\
		\cl2 & \ccc or \ddd UPH; PH \ddd only \\
		\di2 & 3-6, 6+ \hhh \\
		\he2 & 3-6, 6+ \sss \\
		\sp2 & \exactshape{1444} \\
		\cl3/\ddd/\hhh & \shape{4441} short in bid suit (no change)\\
		2NT & ??? \\
		\sp3 & AKQ 7th any suit. (no change) \\
	\end{bidtable}

	Over \cl1--\he1 if opener wishes to give an inverted relay response, 1NT=either major and \cl2=either minor with the same responses as over \cl2. (That is, over the puppet the first step is all lower suit hands, the higher steps are TaJ for the higher suit.  TaJ applies over the lower suit response.)
	...
	For responses at \di2 or above, opener may skip the relay step to zoom into CIRKLE directly. The known length suit is ``A'' for this purpose, other suits are BCD up-the-line.  (For \sp2, \heartsuit is A and \spadesuit is B, per normal rules.)
	...
	\begin{bidtable}{\orauction{1c,1h}}
		\sp1 & Waiting bid. Most common bid by opener, retaining captaincy. \\
	...
	\begin{bidtable}{\orauction{1c,1h,1s}}
		1NT & \sss \\
		\cl2 & \ccc or \ddd, \di2 asks. Over the ask \he2 is all club hands (with \sp2=TaJ) and higher is TaJ responses for \ddd. This is as per \cl1--\cl2. \\
		\di2 & Balanced, 4 \hhh. \\
		\he2 & Balanced, 4 \sss, not 4 \hhh. \\
		\sp2 & Balanced, no 4CM. \\
		2NT & \exactshape{1444} \\
		3x & \shape{4441} stiff in bid suit. (Not \sp3) \\
		\sp3 & ? (Still AKQxxxx, but now with more stuff?) \\
	\end{bidtable}
...
\hhh ~or balanced GF. Over the 1NT waiting bid, \cl2 shows \hhh (with TaJ) and \di2/\hhh/\sss are balanced hands with transfer Stayman style responses. 

By an UPH the hearts hand still has the extra values step, as we avoid bidding \he1 to try to avoid wrong siding and to allow the symmetry for responses. This is the only positive response which retains the extra values step for TaJ.
	
	\subsection{Balanced Follow Ups}	
After \cl1--\sp1--1NT, \di2, \he2 and \sp2 are balanced hands.  We are using CIRKLE responses instead of controls.  \rem{Anyone not using CIRKLE yet can retain the old structure.}

\begin{bidtable}{\orauction{1c,2d}}
	\he2 & Agrees \hhh, asks CIRKLE. \\
	\sp2 & Shows 4+ spades, asks for support. 2NT by responder shows 4+ \sss, over which \cl3 is CIRKLE. Bids above 2NT are suitless CIRKLE responses. \\
	2NT & Denies a major, suitless CIRKLE responses. \\
	\cl3--\di3 & Natural \\
	\he3 & No slam interest, 4 \hhh, choice of games. \\
	\sp3 & No slam interest, exactly 4 \sss, COG.  Rarely used. \\
	3NT & To play \\
\end{bidtable}

\begin{bidtable}{\orauction{1c,2h}}
	\sp2 & Agrees \sss, asks CIRKLE. \\
	2NT & Denies a fit, suitless CIRKLE \\
	\cl3--\he3 & Natural \\
	\sp3 & No slam interest, exactly 4 \sss, COG \\
	3NT & To play \\
\end{bidtable}

\begin{bidtable}{\orauction{1c,2s}}
	2NT & Suitless CIRKLE \\
	\cl3 & Nat, but considering making this a shape relay. See below. \\
	Other & Nat \\
\end{bidtable}

\begin{noted}[Minor Suit Shape Relay]
	In the past I have played \cl3 as asking with \di3 as a 5332 either minor (+1 for LH), 3M as doubleton in 2344, 3NT any 4333. Ari mentioned whether we have a shape search over this bid during play in the JLall so adding this comment here for discussion.
\end{noted}
	...
	\di2/\hhh are xfers, showing 6+ in the major and about 3--6 HCP.  Accepting the xfer is NF, other bids force 1 round.
	
	\subsection{3 suiters}
	
	\sp2 thru \he3 are 3 suited hands with shortness in the bid suit. Over these 4x1 bids, every suit can be agreed below game. 3NT to play, agreeing a suit triggers CIRKLE for that suit.
	...
	\rem{Changing this to an agreement, but can be removed later if desired. 14 Apr 2021}
